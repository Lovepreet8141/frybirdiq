...
**This is the important one.** Research for the Command Center spec (see `design-system/pages/iq.md`) found that `[data-surface="iq"]` is applied at the top level in `src/app/(app)/app/layout.tsx` — wrapping every route under `/app/*`, POS included — and fully overrides every token `.surface-dark` sets, at equal CSS specificity, later in the cascade. **POS is very likely rendering in the light IQ palette rather than the dark theme its own design intent calls for** (`.surface-dark`'s own comment: staff look at this screen for a whole shift, a cream field is fatiguing). The CSS mechanism is unambiguous by direct inspection; the actual rendered result was **not visually confirmed — no browser tooling available this session**. Before touching POS theming, load `/app/pos` on a real device or a browser and look. If confirmed, this is a live UX defect on the single most-used staff screen in the product, not a documentation mismatch.
