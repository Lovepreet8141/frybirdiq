...
export const IQ_EXECUTION_MODES = ["AUTO", "APPROVAL", "HANDOFF"] as const;
...
export const iqActions = pgTable(
  ...
  (table) => [
    ...
    // state-machine.ts MODES_FOR_TIER.
    check(
      "iq_actions_tier_mode_check",
      sql`(${table.tier} = 'A0' AND ${table.mode} = 'AUTO')
          OR (${table.tier} = 'A1' AND ${table.mode} IN ('AUTO', 'APPROVAL'))
    ...
    check(
      ...
      sql`CASE WHEN ${table.tier} = 'A1' AND ${table.mode} = 'AUTO'
            THEN ${table.autoPolicyId} IS NOT NULL AND ${table.autoPolicyVersion} IS NOT NULL
            ELSE ${table.autoPolicyId} IS NULL AND ${table.autoPolicyVersion} IS NULL END`,
    ),
