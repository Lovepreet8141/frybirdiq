...
export interface RememberedIdentity {
  ...
  /**
   * The orders this browser placed, per a cookie verified against COOKIE_SECRET (cookie-sign-1). There is no unsigned
   * form any more (`cookie-secret-dependency`): a `RememberedIdentity` is never held without one, so ownership is always
   * decided by this list, never by `phone` alone.
   */
 ...
 * phone on those orders alone — never on a bare phone-number match.
 */
export function rememberedPhoneFor(orderId: string, remembered: RememberedIdentity | null): string | null {
  return remembered?.orderIds.includes(orderId) ? remembered.phone : null;
}
...
export function viewerOwnsOrder(
  ...
  return !!order.id && !!remembered?.orderIds.includes(order.id);
}
