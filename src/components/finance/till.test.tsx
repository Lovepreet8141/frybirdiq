...
describe("TillPanel", () => {
  ...
  it("every form carries an idempotency key, and the handover form states the amount owed before the rider's count is typed", () => {
    const rider = { riderUserId: "r1", riderName: "Vikram", paymentCount: 2, amount: fromRupees("800"), since: "2026-09-20T06:00:00Z" };
    const html = renderToStaticMarkup(<TillPanel open={open} recent={[]} riders={[rider]} canManage />);
    expect(html.match(/name="idempotencyKey"/g)).toHaveLength(2); // close + handover
    expect(html).toContain("Amount owed by Vikram");
    expect(html).toContain("₹800");
    expect(renderToStaticMarkup(<TillPanel open={null} recent={[]} riders={[]} canManage />).match(/name="idempotencyKey"/g)).toHaveLength(1); // open
  });
...
describe("ReconciliationTable", () => {
  ...
  it("a day withheld for the open till shows no cash figures, only that they appear when the till closes", () => {
    const html = renderToStaticMarkup(<ReconciliationTable days={[{ ...day, cashHidden: true, cashInTill: paise(0), cashRefunded: paise(0), net: paise(0) }]} openSession periodLabel="Today" />);
    expect(html).toContain("Net shown when the till closes");
    expect(html).toContain("Hidden until the till closes");
