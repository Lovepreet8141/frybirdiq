...
export async function recordExpense(
  ...
  // analytics.view, which is read-only and held by ANALYST, and no longer
  // finance.view, which is the till-ledger read. Same roles today (OWNER,
  // MANAGER); the point is that widening who may read the ledger can no longer
  // silently widen who may write the books.
...
export async function setFoodCostTarget(
  ...
  // A food cost target is a figure the P&L measures against, so setting it is a
  // finance write. finance.manage, not the ledger read. See recordExpense.
  if (!(await staffCan("finance.manage"))) {
