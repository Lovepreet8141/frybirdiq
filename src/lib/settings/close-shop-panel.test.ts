...
vi.mock("@/lib/orders/shop-status-actions", () => ({ pauseOrderingAction: vi.fn(), resumeOrderingAction: vi.fn() }));

import { CloseShopPanel, type CloseShopPanelProps } from "../../app/(app)/app/admin/restaurant/close-shop-panel";

const restartLines = { UNTIL_NEXT_OPENING: "Orders restart tomorrow at 11:30 AM.", UNTIL_RESUMED: "Orders stay off until you switch them back on." } as const;
const render = (props: Partial<CloseShopPanelProps> & Pick<CloseShopPanelProps, "status">) =>
  renderToStaticMarkup(createElement(CloseShopPanel, { restartLines, pausedSince: null, ...props }));

describe("CloseShopPanel", () => {
  it("open: says OPEN, offers to switch off, and shows the orders still due", () => {
    const html = render({ status: { state: "open", closesAt: "23:00", pausedBy: null, reason: null, ordersStillDue: 2 } });
  ...
  it("paused: says CLOSED with who, when, reopens, reason, and offers only to switch back on", () => {
    const html = render({
      status: { state: "paused", mode: "UNTIL_RESUMED", pausedAt: new Date("2026-09-19T14:12:00Z"), reopensAt: null, reopensAtLabel: null, withinHours: true, pausedBy: { userId: "u", name: "Aman" }, reason: "Fryer broken", ordersStillDue: 0 },
  ...
  it("paused until next opening: shows the reopen time", () => {
    const html = render({
      status: { state: "paused", mode: "UNTIL_NEXT_OPENING", pausedAt: new Date(), reopensAt: new Date(), reopensAtLabel: "tomorrow at 11:30 AM", withinHours: true, pausedBy: { userId: "u", name: null }, reason: "Power cut", ordersStillDue: 1 },
      pausedSince: "today at 7:42 PM",
    });
  ...
  it("closed by hours: says CLOSED and when it reopens", () => {
    const html = render({ status: { state: "closedByHours", reopensAt: new Date(), reopensAtLabel: "today at 11:30 AM", pausedBy: null, reason: null, ordersStillDue: 0 } });
    expect(html).toContain("Shop is CLOSED for orders");
