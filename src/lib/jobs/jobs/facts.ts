...
/** Locked steps per day — facts, then trust — each of which may use the whole wait budget. */
...
/**
 * The wait budget for each locked step of one day, or null when there is not
 * enough time left to start the day. Every step may wait MAX_LOCK_WAITS times,
 * so all waits of all steps together stay inside the deadline less the reserve.
 */
export function dayLockBudget(remainingMs: number): DayLockBudget | null {
  const perWait = Math.floor(Math.min(MAX_LOCK_WAIT_MS, (remainingMs - LOCK_BUDGET_RESERVE_MS) / (MAX_LOCK_WAITS * LOCKED_STEPS_PER_DAY)));
