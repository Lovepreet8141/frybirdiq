...
const GATED: Readonly<Record<string, readonly string[]>> = {
  ...
  "src/lib/auth/staff-actions.ts#postRiderPositionAction": ["delivery.complete"],
...
const PUBLIC: Readonly<Record<string, string>> = {
  ...
  "src/lib/auth/actions.ts#requestPasswordResetAction": "the shared staff/customer password-reset flow (auth-v3); never reveals whether the address has an account",
  "src/lib/auth/actions.ts#verifyResetCodeAction": "the shared staff/customer password-reset flow (auth-v3); Supabase's own verifyOtp is the authority",
  "src/lib/auth/actions.ts#setNewPasswordAction": "auth-v3: opened to customers too, by owner decision — getUser() decides whose password changes; getStaff()/getCustomer() only pick the audit label and whether the owner alert fires, never whether the action runs",
  ...
  "src/lib/customer/actions.ts#createAccountAction": "the public signup form; never reveals whether the email already has an account (Supabase's own signUp() anti-enumeration, plus onConflictDoNothing on the customers insert)",
  "src/lib/customer/actions.ts#resendSignupCodeAction": "resends the signup-confirmation code; never reveals whether the address has an account",
  "src/lib/customer/actions.ts#verifySignupCodeAction": "checks the signup-confirmation code; Supabase's own verifyOtp is the authority",
  "src/lib/customer/actions.ts#signInCustomerAction": "customer password sign-in itself: it is how a session is obtained",
