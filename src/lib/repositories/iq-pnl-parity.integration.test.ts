...
import { businessDatesOf, readDailyFacts, recomputeDay } from "./iq-facts";
...
describe("IQ daily facts — P&L parity (IQ-1 S6)", () => {
  ...
  it("is idempotent, and serialises concurrent recomputes of the same day", async () => {
    const rowsFor = async () =>
    ...
    await Promise.all([recomputeDay(orgs.a.orgId, "2026-09-01"), recomputeDay(orgs.a.orgId, "2026-09-01"), recomputeDay(orgs.a.orgId, "2026-09-01")]);
