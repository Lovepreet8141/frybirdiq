...
## 38. release/rc-35-rider-live reviewed, awaiting go/no-go (22 Sep 2026)

- **Built and reviewed on `release/rc-35-rider-live`** (worktree `~/Downloads/wt-live`, on top of `release/rc-34-security-lows`): rider live position. The rider's browser posts a GPS fix every 15 s while a delivery is OUT_FOR_DELIVERY (`postRiderPositionAction`, `delivery.complete`, the rider is always the signed-in person, never the form); the customer's `/order/[id]` page shows the newest fix on a Leaflet/OSM map only while it is out, only to the order's owner (`viewerOwnsOrder`, unchanged); fixes are kept 24 hours then deleted by the new `rider-positions-purge` job (hourly). Migration `0056_rider_positions`: RLS forced, ALL privileges revoked from `PUBLIC`/`anon`/`authenticated`, no client policy at all — the table is unreachable through PostgREST/Realtime by any client role, verified read-only and by test.
- **Security: APPROVE. Red-team: AGREE.** No blocker/high/medium finding against the new code. Both flagged the same pre-existing, currently-inert low: the legacy *unsigned* remembered-contact cookie (only reachable if `COOKIE_SECRET` is ever unset) authorizes by phone number alone; this feature raises its stakes, since it would also expose a rider's live location and the destination pin, not just receipt fields. Not a bug in this branch and not currently reachable (`COOKIE_SECRET` has been set in production since Release 19a) — logged here as a reason that secret must never be unset. Low card: `cookie-secret-dependency`.
- **Migration undo drilled** on a schema built from 0000-0055 with RLS/grants in the fingerprint: exact, re-apply identical, zero client grants confirmed after apply.
- **Gates green** on the branch (typecheck, lint, unit and integration).
- **Awaiting the owner's go/no-go** before any deploy (customer-facing, touches order data — gated per the sprint rule).
