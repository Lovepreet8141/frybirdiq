...
## 45. Card `alert-owner-created` — built, on its own unmerged branch, waiting for a go/no-go

Sent separately, not repeated here in full: audit row + alert (journal or `ALERT_URL`) when an OWNER membership is created or reactivated. Branch `release/rc-39-alert-owner-created` (pushed), based on `kit-radix-nova` @ `c6c5115`. Not merged, not deployed — this HANDOVER file on `kit-radix-nova` therefore does not yet carry that branch's own §45 entry; read it there if picking this up fresh. No conflict expected with the card below (different files: `staff.ts` / `src/lib/auth/` vs `src/app/api/whatsapp/`).

## 46. Card `whatsapp-direct` part 1 — webhook endpoint — built, waiting for the owner's go/no-go (24 Sep 2026)

**Production, unchanged since §44:** live commit `1c7a5e6`, BUILD_ID `866c6a9dc872486d`, 57/57 migrations, fingerprint `4a3d000a67a52b7e857e320f2d79df2b`. Nothing below is deployed, merged or committed to `kit-radix-nova`.

- **What:** `GET /api/whatsapp/webhook` answers Meta's one-time (and any re-triggered) verify handshake — `hub.mode` must be `subscribe` and `hub.verify_token` must match `WHATSAPP_VERIFY_TOKEN` exactly, in constant time; on a match, `hub.challenge` is echoed back as plain text; anything else, including the env var being unset, is 403. `POST /api/whatsapp/webhook` verifies `X-Hub-Signature-256` (HMAC-SHA256 over the exact raw body, `WHATSAPP_APP_SECRET`, constant-time) before anything is parsed; unverified is rejected, verified is answered 200 immediately. **Nothing is stored.** The only thing ever logged is the event's `object` (e.g. `whatsapp_business_account`) and each change's `field` (e.g. `messages`) — never `value`, which is where Meta puts phone numbers and message text. No customer message is read or sent anywhere in this card; that is a later, separate one.
- **Fail-closed, both directions:** either env var missing means every request to this route is refused, not silently trusted or silently ignored — proven with a deliberate-break-then-restore fail-first pass (see Tests below).
- **Defence in depth on size and rate**, per the request:
  1. **nginx** (`deploy/nginx-frybird.conf`, VPS change, not yet applied): a new `location ^~ /api/whatsapp/webhook` — `client_max_body_size 512k` (a real Cloud API payload is a few KB) and `limit_req zone=whatsapp_webhook burst=10 nodelay` (5 r/s per source IP, a new `limit_req_zone` declared alongside the existing `map` block). No `proxy_set_header` of its own, so it inherits the six the server block already sets (RED-TEAM F2's class of bug: a location that sets even one gets none of the inherited ones).
  2. **The app itself**, independently: `MAX_BODY_BYTES = 262_144` (256 KiB) checked against both the `Content-Length` header (a claim) and the actual body length after reading (the guarantee) — 413 either way, before signature verification runs.
- **Middleware:** `src/proxy.ts`'s matcher now also excludes `api/whatsapp/webhook(?:/|$)`, the same way `api/jobs` and `api/health` already are — Meta carries no session to refresh, and this route must answer fast without paying for, or depending on, a Supabase round trip. (The proxy never blocked or redirected anything for `/api/*` routes even before this — it only refreshes cookies — so this exclusion is about speed and independence from Supabase being up, not about unblocking something that was blocking Meta.)
- **Files:** `src/lib/notifications/whatsapp-webhook.ts` (new, pure — `verifyHandshake`, `verifySignature`, `eventTypesOf`, `whatsappVerifyToken`/`whatsappAppSecret` read directly from `process.env`, the same pattern as `razorpayConfig()`, not through `serverEnv()`, since this route never touches the database) + its unit test (17 tests); `src/app/api/whatsapp/webhook/route.ts` (new, thin — GET/POST handlers, body-size guard, `dynamic = "force-dynamic"`); `src/proxy.ts` (matcher) + its two existing test files extended (`src/lib/auth/proxy-matcher.test.ts`, `src/lib/health/proxy-matcher.test.ts`); `deploy/nginx-frybird.conf` (new location + `limit_req_zone`); `.env.example` (`WHATSAPP_VERIFY_TOKEN`, `WHATSAPP_APP_SECRET`, and — a pre-existing gap, fixed in passing — `WHATSAPP_UPDATES`, which had no `.env.example` line at all despite being read by `whatsappUpdatesEnabled()` since roadmap 7.2).
- **No migration.** No database code touched at all.
- **Branch:** `release/rc-40-whatsapp-webhook` (pushed once committed), based on `kit-radix-nova` @ `c6c5115` — independent of, and not stacked on, `release/rc-39-alert-owner-created` (§45); the two touch disjoint files. Not merged, not deployed.
- **Tests / fail-first:** unit 194 test files / 3230 tests total (was 193/3208 at Release 26 — one new file, `src/lib/notifications/whatsapp-webhook.test.ts`, 17 tests; plus 5 more assertions added to the two existing proxy-matcher test files). The new file covers both fail-closed cases (`configuredToken`/`appSecret` unset), wrong mode, missing/wrong/prefix-mismatched token or signature, the exact-raw-body requirement (a signature for a different body is refused), case-insensitive hex comparison, and `eventTypesOf` never throwing on malformed/unexpected JSON while never surfacing `value`. Fail-first proven directly: temporarily replaced the constant-time, fail-closed comparisons with naive `===` checks and no guards — 5 of the 17 tests failed exactly as expected (the two fail-closed cases, the wrong-mode case, the missing-header case, the no-prefix case); restored, all 17 green again. Integration suite unaffected (83 files / 843 tests, unchanged — no DB code in this card).
- **Gates:** typecheck clean, lint 0 errors (same 19 pre-existing unrelated warnings), `check:rsc-boundaries` clean. No route-level test file — `vitest.config.mts`'s `include` is scoped to `src/lib`, `src/domain`, `src/db`, `src/components` on purpose (framework glue under `src/app` is never unit-tested here, matching every other route in the repo, e.g. the Razorpay webhook route has no test file either — all its logic lives in and is tested via `src/lib/payments/razorpay.ts`).
- **No reviewer has looked at this yet** — public, unauthenticated, signature-gated surface is exactly the kind of thing that's worth a real security pass even though nothing here touches money, orders or the database; sent as a go/no-go per the owner's own instruction rather than shipped autonomously.
- **What ships live vs. what's still off:** the endpoint exists and answers correctly the moment this deploys, but it is functionally inert with both env vars unset (every request 403s) — deploying this card changes nothing observable until the owner also (a) types `WHATSAPP_VERIFY_TOKEN` and `WHATSAPP_APP_SECRET` into `/etc/frybird/env` on the VPS himself and restarts, and (b) points Meta's App Dashboard at the Callback URL. Both are the owner's own steps, listed in the go/no-go sent alongside this entry.
- **Known limits, stated for the go/no-go:** (1) the nginx rate limit is per-source-IP; Meta may deliver from a shared/rotating IP range, so a very unlucky clustering of unrelated Meta traffic through the same edge IP could in theory hit the 5 r/s + burst-10 ceiling — no evidence this is a real risk at one restaurant's volume, flagged as a card to revisit (`whatsapp-webhook-rate-1`) only if Meta's dashboard ever shows delivery failures; (2) no retry/backoff logic is needed here since Meta itself retries failed deliveries on its own schedule and this route is idempotent-by-design (nothing is written, so a re-delivered event is just logged twice); (3) the `console.log` line is INFO-level noise on every verified event — acceptable for now (there is no volume yet), revisit if it gets noisy once real traffic exists.
- **Next:** waiting on the owner's yes; on a yes, this needs a security-adjacent review (public unauthenticated endpoint) before merge and deploy — normal quiet-shop check + fresh dump, no migration so no DB-side risk.
