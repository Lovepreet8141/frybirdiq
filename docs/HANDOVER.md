...
## 45. Card `email-otp` built, reviewed, waiting for the owner's go/no-go (24 Sep 2026)

**Production, unchanged since §44:** live commit `1c7a5e6`, BUILD_ID `866c6a9dc872486d`, 57/57 migrations, fingerprint `4a3d000a67a52b7e857e320f2d79df2b`. Nothing below is deployed, merged, committed to `kit-radix-nova`, or pushed anywhere except its own branch.
...
## 46. Card `otp-animation` built and reviewed, on top of §45 (24 Sep 2026)

**Production, unchanged since §44/§45.** Nothing below is deployed, merged, or pushed except to its own branch. This card is presentation-only, on top of the still-unshipped `email-otp` card, so neither can go live before the other.

- **Item 1 (code length check, done first):** `verifyOtpAction`'s server-side validation was already correctly 6 digits (`z.string().regex(/^\d{6}$/)`) — never a "4" bug. But the shipped UI on `release/rc-41-email-otp` was a single free-text input (`maxLength={6}`) with a manual "Sign in" button, not 6 discrete boxes and not auto-submitting — which didn't meet what this card actually needs. Fixed in a new commit on `release/rc-41-email-otp` (`c5eeaf8`) before starting this card, per the instruction to fix rc-41 first: new `src/components/account/code-boxes.tsx` (`CodeBoxes`) — 6 boxes, one hidden `token` field (server contract unchanged), auto-submits via `form.requestSubmit()` the instant all 6 fill (typed or pasted), remounts (clears + refocuses box 0) on every new verify error via `key={resetToken}`. Re-reviewed: red-team APPROVE, security APPROVE — security also caught a real bug (see below).
- **Real bug found and fixed during review:** the first version called `requestSubmit()` synchronously inside the same handler that called `setDigits(next)` — `requestSubmit()` reads the hidden input's value straight off the live DOM, which React hasn't committed the just-typed/pasted digit to yet at that point in a plain event handler. Would have auto-submitted a token one digit short (or empty, on paste) on every attempt, bouncing to a false "wrong code" error even when the customer typed/pasted it correctly. Fixed by moving the submit call into a `useEffect` keyed on `digits`, which runs after React commits — traced through the actual installed `react-dom` build by the reviewing agent to confirm the fix, not just assumed. Delta re-verified: correct, no new dev-only double-submit, no race with the reset-on-error remount.
- **Item 2 (mail icon):** the "Check your email" panel's icon is now Lucide `Mail`, not the zip's Gmail-logo PNG.
- **Item 3 (purposeful motion only):** three new one-shot CSS animations in `globals.css` — `.otp-shake` (code-box group, transform-only shake on a wrong code), `.otp-box-flash` (a decaying red glow ring per box, paired with the shake), `.otp-tick-pop` (the success checkmark popping in). All finite (no `infinite`), all gated on a prop that's only ever true at the exact moment the box group remounts after an error, so they play once and never replay just from the customer typing. A new `VerifyStatusIcon` (reads `useFormStatus().pending` and `verifyState.status`, no new server state) shows Mail (idle) → spinning `Loader2` (verifying) → `CheckCircle2` (inferred success — `verifyOtpAction` has no explicit success status at all, since a real success calls `redirect()` server-side and the browser just navigates away; the checkmark is inferred client-side from `pending` going true then false with no error). All three animations, plus the pre-existing `.hero-ticker-slide`, are under `@media (prefers-reduced-motion: reduce) { animation: none }`.
- **Item 4 (small phones):** compact by default, not just hidden past a breakpoint — the explanatory panel shrank from a big centered icon+heading+paragraph block to one inline row, and moved inside the `<form>` so it could read live state. A `@media (max-height: 500px) { .otp-status-row { display: none } }` rule drops that row entirely as a second line of defense on a genuinely short viewport (a small phone with the keyboard open), so the code boxes and submit button stay on screen without scrolling either way. Known caveat, disclosed: this is a `max-height` layout-viewport query, which is reliable on Android Chrome but not guaranteed on iOS Safari, where the layout viewport doesn't always shrink with the keyboard — the compact-by-default sizing is the real fix, this media query is a bonus, not the only line of defense.
- **Item 5 (performance):** never built the conic-gradient/blurred-border spinner from the original zip at all — I have no way to run Chrome DevTools' CPU throttle profiler in this environment, so rather than build something risky and ask the owner to test-then-maybe-replace it, the "verifying" indicator is the same `Loader2` + `animate-spin` pattern already shipped live in this exact codebase on every form's own submit button (`Submit`, `ResendCodeButton`) — proven cheap by already being in production, not assumed. The shake/flash/tick-pop animations are `transform`/`opacity`/`box-shadow` only, one-shot, on one or a handful of small elements — no continuous heavy effect anywhere in this card.
- **Item 6 (tokens/deps/logic):** every color is an existing semantic token (`--destructive`, `--success`, `--primary`, `--border`) read through Tailwind's existing utilities — no new token, no new dependency (`lucide-react` `Mail`/`CheckCircle2` were already a project dependency), no server logic changed (`src/lib/customer/actions.ts` has zero diff across this entire card, confirmed by both reviewing agents independently via `git diff`, not taken on my word). `python3 scripts/check-contrast.py` re-run after the color-adjacent CSS changes: all pairs within their stated limits, no new failing pair introduced.
- **Reviews, both with real tool use, in two rounds** (the rc-41 box-mechanics fix, then this card's remaining UI diff):
  - Red-team on the rc-41 fix: **APPROVE.** Traced the `submittedRef` guard through real event-timing reasoning (paste storms, autofill, the `pending`/ref race) and found no duplicate-submit path; confirmed the remount-on-error clears state correctly on consecutive errors (verified `verifyOtpAction` returns a fresh object literal every time, so React's reference-equality re-render check fires reliably); confirmed `src/lib/customer/actions.ts` has zero diff. Two non-blocking notes: an unreachable-in-practice defensive-coding nitpick on an optional-chained `requestSubmit()`, and "Resend code" not clearing the boxes (pre-existing behavior from the old single-input version, not a regression, not a security issue since the server re-validates whatever token is actually submitted).
  - Security on the rc-41 fix: **found the stale-DOM auto-submit bug above** (traced through the actual installed `react-dom-client.development.js` source, not assumed), confirmed everything else clean (server validation/rate-limiting untouched, no XSS/injection surface, paste handler uses only user-gesture-gated `getData`, no unexpected clipboard access). Delta re-check after the fix: **APPROVE**, fix confirmed correct by tracing the same react-dom commit-phase ordering (mutation phase writes the DOM before passive effects run), confirmed Strict Mode's dev double-invocation doesn't apply once `digits` is genuinely non-empty, all four gates re-run clean.
  - Security on the remaining UI diff (`auth-forms.tsx`, `globals.css`): **APPROVE, no finding.** Specifically traced whether `VerifyStatusIcon` could ever show a false-positive success — it cannot (`pending` unconditionally wins first); found the opposite, cosmetic-only asymmetry instead: a retry-after-failure that then succeeds keeps showing the idle Mail icon instead of the checkmark, since `verifyState` never gets overwritten by a real success (it redirects instead of returning a new state) so the stale `invalid=true` lingers — the actual redirect still fires correctly regardless, purely a missed visual flourish, not fixed. Confirmed the JSX reorganization didn't change what error messages show, when, or in what order.
- **Gates (final, on the full card):** typecheck clean, lint 0 errors (same 19 pre-existing unrelated warnings), unit 195 files / 3238 tests, rsc-boundaries clean, full production build clean, `check-contrast.py` all pairs within limits.
- **Not covered by any automated test:** the interactive/timing logic in `code-boxes.tsx` (auto-submit, paste-fill, remount-on-error) has zero test coverage — this repo's `pnpm test` convention is pure `src/lib`/`src/domain` logic only (no jsdom/testing-library anywhere in the project), and adding a new test framework for one component would violate this card's own "no new dependencies" rule. **Required before deploy, live on frybirdiq.tech, not localhost:** type all 6 digits one at a time in a fresh session and confirm it auto-submits and signs in on the first try; separately, request a new code and paste the full 6-digit code into an empty box and confirm the same. Both flagged explicitly by the security reviewer as the one thing that couldn't be verified by source-reading alone.
- **Previews (both live, private, mine to share):**
  - The original supplied design (`otp v4.zip`), hand-recreated from its source for review, not executed: `https://claude.ai/artifact/B7kY5Umdgoy7jnEAcugzK4`
  - This rebuild's four states (idle, verifying, wrong code, success) at phone (360×640, keyboard-open simulated) and desktop (the real 384px `max-w-sm` card) widths, using the real customer-site tokens: `https://claude.ai/artifact/A8FbqQZ9yysqAtrW5teySi`
- **Unrelated aside surfaced while building the preview, not touched, out of scope for this card:** `globals.css`'s own comment says "the customer site is cream," and no `.dark` class is applied anywhere in `(site)/layout.tsx` or the root layout — the customer site currently renders in the light cream palette, not dark, contradicting CLAUDE.md's "the customer site is dark" line. Worth someone confirming which is actually intended.
- **Branch:** `release/rc-42-otp-animation`, based on the fixed `release/rc-41-email-otp` @ `c5eeaf8`. Not merged, not deployed — and cannot ship before `email-otp` (§45) does, since it depends on rc-41's box-entry fix.
- **Next:** waiting on the owner's review of both preview links and the live hand-test above, then a go/no-go — same auth-adjacent rules as §45, plus this card's own dependency on §45 shipping first.
