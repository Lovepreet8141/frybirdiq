...
## 45. Card `alert-owner-created` built, waiting for the owner's go/no-go (23 Sep 2026)
...
- **What:** when an OWNER membership is created or reactivated (`inviteStaff` — a fresh invite, or a resend that reactivates a deactivated OWNER row; `changeStaffRole` — promoting someone straight to OWNER, or reactivating their dormant OWNER row), the write now also: (1) inserts a dedicated audit row in the SAME transaction as the membership write (`owner_membership_created` / `owner_membership_reactivated`, entity `memberships`, so it can never be lost to a failed alert), and (2) after the transaction commits, calls `sendOwnerMembershipAlert` (new module `src/lib/auth/owner-membership-alert.ts`) — POSTs a short plain-text line to `ALERT_URL` if the owner has set one, otherwise writes the same line to the journal (`console.error`, same convention as `cookieSecret()`'s missing-secret alert). Never throws: a delivery failure cannot turn an allowed OWNER invite/promotion into a 500.
- **Why a second, app-level `ALERT_URL`:** the existing `ALERT_URL` (DEPLOY.md §11.2) lives in `/etc/frybird/alert.env` and is read only by systemd's `frybird-alert@.service` (OnFailure of the backup and job units) — the Next.js process never sees it. This card's alert fires from inside a request (a staff member invited/promoted someone), not from a failed systemd unit, so it needs its own copy of the same kind of value in the app's own env file. Added as `ALERT_URL` to `serverEnv()` (optional, credential — never logged) and to `.env.example`. **Nothing is required from the owner to ship this**: unset, every OWNER-membership event still lands in the journal.
- **No migration.** `audit_logs.action` is free text (existing table, existing column); the new audit actions need no schema change.
- **Branch:** `release/rc-39-alert-owner-created` (pushed once committed), based on `kit-radix-nova` @ `c6c5115` (current tip, carries Release 26 / §44). Not merged, not deployed.
- **Files:** `src/lib/auth/owner-membership-alert.ts` (new, pure alert sender) + its unit test (5 tests: journal fallback, POST on `ALERT_URL` set, URL never logged, HTTP-failure fallback, network-failure fallback — none throw); `src/lib/env/index.ts` (`ALERT_URL` added to `serverSchema`); `.env.example`; `src/lib/repositories/staff.ts` (`inviteStaff`, `changeStaffRole` — both now track an `ownerEvent` set inside the transaction, audited inside it, alerted after it commits); new integration test file `src/lib/repositories/staff-owner-alert.integration.test.ts` (8 tests against the real local database, covering both create-paths and both reactivate-paths, plus three negative cases: a non-OWNER invite, a non-OWNER role change, and a refused change — none of which should write an owner audit row or call the alert sender).
- **Fail-first proven:** stashed just the `staff.ts` change and re-ran the 8 new integration tests against the pre-fix code — the 4 positive-case tests failed exactly as expected (no audit row written), the 4 negative-case tests still passed (nothing changed for them either way). Un-stashed and re-ran green.
- **Gates on the working tree (no migration, so no DB-schema gate applies):** typecheck clean; lint 0 errors (the same 19 pre-existing unrelated warnings); unit 194 test files / 3212 tests (was 193/3208 before this card); integration 84 test files / 851 tests (was 83/843); `check:rsc-boundaries` clean.
- **No reviewer has looked at this yet** — it touches auth-adjacent code (staff.ts), so per the owner's own instruction this card is sent as a go/no-go before any reviewer or deploy step, not shipped autonomously even though it needs no migration.
- **Known limits, stated for the go/no-go:** (1) `ALERT_URL` is unset in production, so until the owner sets one every event lands only in the journal (`journalctl`), not on a phone — same gap as the still-not-installed DEPLOY.md §11.2 alerting; (2) the alert fires once per event, best-effort, with a 15 s timeout and no retry (unlike `deploy/notify.sh`'s 2 retries) — a transient failure is only ever visible in the journal, never re-sent; (3) `deactivateStaff` is untouched by design — deactivating an OWNER is the opposite direction and already writes its own `staff_deactivated` audit row; (4) no attempt to alert on an OWNER row created by `pnpm staff:grant` (`scripts/grant-role.ts`), the one write path this module doesn't cover — flagged, not fixed, since that script is a deliberate out-of-band escape hatch already noted in `staff.ts`'s own header.
- **Next:** waiting on the owner's yes; on a yes, this needs no reviewer beyond a normal auth-adjacent pass (the diff never changes who can be invited or promoted — `canGrantRole`'s ceiling is untouched — it only adds a write and a notification after a change `canGrantRole` already allowed).
