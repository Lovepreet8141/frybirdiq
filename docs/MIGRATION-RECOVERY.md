...
## 4a. 0034 `iq_foundations` (not deployed; `agent/database-iq0-s6`, card iq0-s6)

Added 2026-09-17. Requires 0033. Production run is owner gate 1 and
decision `dec-2`.
...
- **Journal (FACT):** drizzle-kit stamped `when` 1789615256699, which is
  lower than 0033's 1789700000000; `db:migrate` would have skipped 0034 in
  production without an error. Set by hand to 1789800000000.
  `src/db/schema/iq.test.ts` now fails on any journal entry that does not
  increase.
...
- [ ] Open: SECURITY-TENANCY rules on ADMIN reading `iq_insights` food-cost
      figures without `finance.view` (DESIGN-v2-DELTA "Open"). The policy
      follows the design (OWNER, ADMIN, MANAGER) until then.

### Hand edits drizzle-kit does not know about (for the next `db:generate`)
`iq_insights_superseded_by_fk` is `DEFERRABLE INITIALLY DEFERRED`; the two
