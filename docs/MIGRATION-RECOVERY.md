...
## 4h. 0041 `cash_sessions` (roadmap 5.1-5.3, `release/rc-22-cash-sessions`)

- **What:** tables `cash_sessions` (float, counted, expected snapshot, variance, who opened/closed; one OPEN per org+location by a partial unique index; CHECKs keep an open row free of closing facts and a closed row's variance equal to counted minus expected) and `cash_handovers` (a rider's declared cash against the books', into an open session); four columns on `payments`: `cash_session_id`, `collected_by`, `held_by_rider` (default false), `handover_id`; RLS on and forced, no client DML. Journal `when` 1790600000000.
- **Expand-only:** two empty tables and four nullable/defaulted columns the live code never names. Every existing payment reads as "not in any session, not held by a rider". Applying it before the deploy is safe.
- **Classification: reversible while no till history exists.** `supabase/rollback/0041_cash_sessions.down.sql` REFUSES while any cash session or handover exists (they are the till's only record: who was short, by how much, which rider handed over what). Export and delete deliberately first. Drilled on a throwaway database: apply, existing rows default correctly, refuse with a session present (schema untouched), undo, re-apply. Delete the 0041 row from `drizzle.__drizzle_migrations` (created_at 1790600000000).
- **Code-only rollback (to the previous build) is safe for the database:** the previous build's full integration suite passes with 0041 present. After the rollback nothing attaches cash payments to a till any more and rider cash is no longer marked as held; the tables and columns keep what they had. Re-deploying the new build resumes from there. Cash taken while rolled back is simply "not in any session" and shows in the reconciliation as such.
...
## 4i. 0042 `rls_role_reads` (p0-7 defect 2, `release/rc-23-p0-7`)

**What it is.** 40 restrictive SELECT policies (`<table>_role_read`, `AS RESTRICTIVE FOR SELECT TO authenticated`) on customers, payments, refunds, the books, orders, stock, recipes, integrations, devices, ai_* and `memberships`. A restrictive policy is ANDed with the existing permissive one, so a staff login using the anon key through PostgREST or Realtime now reads only what its role's permissions name (`src/domain/rls-read-limits.ts`, generated by `scripts/gen-rls-read-policies.ts`, drift-tested). Menu and configuration tables are untouched. The app itself reads through Drizzle as `postgres` (bypasses RLS): no page or action changes.

**Classification.** Additive (policies only; no table, column, grant, row or existing policy is changed). Undo drilled on a real-schema throwaway database built from migrations 0000-0041: apply adds exactly 40 policies and touches no other; the undo restores the policy set byte-for-byte (fingerprint identical); re-apply works.

**Code-only rollback.** Safe and irrelevant: the code never depends on these policies. The policies only narrow what a direct database client can read.

**Undo.** `supabase/rollback/0042_rls_role_reads.down.sql` (hand-run, one transaction, drops the 40 policies, no data touched).

**Risk to watch after deploy.** Realtime `postgres_changes` on `order_events` is read with the subscriber's own token: KITCHEN, RIDER, CASHIER, MANAGER, OWNER and ADMIN keep it (they hold orders.view, orders.create, kitchen.view or delivery.view); INVENTORY and ANALYST do not need it. `products` and `product_availability` stay open to every member.

**4i addendum (review fix).** Both the security and red-team reviewers flagged that `pos_pin_hash` was readable through PostgREST by OWNER and ADMIN logins (row policy only). 0042 therefore also replaces the table-level SELECT on `memberships` for `authenticated` with a column-level grant of every column EXCEPT `pos_pin_hash` (`MEMBERSHIPS_READABLE_COLUMNS`; a test forces a decision for any new column). Undo restores the table-level SELECT (production had it: verified read-only). The undo drill now checks the grant too: pin closed after apply, original grant restored exactly by the undo. Nothing in the app reads `pos_pin_hash` through PostgREST (Drizzle bypasses grants). Low cards from the reviews: `invite-race-1` (concurrent identical invites can raise a unique violation, a 500 not an escalation), `staff-lock-1` (deactivate/role-change read roles without FOR UPDATE; needs a simultaneous OWNER action, grants nothing), `orders-column-limits` (KITCHEN/RIDER/ANALYST can read contact fields on orders through PostgREST: consistent with the permission model, owner decision on role-limited columns).

## 4j. 0043 `rider_assignment` (roadmap 6.3, `release/rc-25-rider`)

**What it is.** `orders.rider_id uuid` and `orders.rider_assigned_at timestamptz` (both nullable, no default), a CHECK that only a DELIVERY order can have a rider (`orders_rider_only_when_delivery`), and a partial index `orders_rider_idx (org_id, rider_id) WHERE rider_id IS NOT NULL`. New permission `delivery.assign` (OWNER, ADMIN, MANAGER) in code only.

**Classification.** Additive. Existing orders read as "no rider assigned". ADD COLUMN with no default is catalogue-only; the CHECK is validated over the existing rows (about a hundred, all with `rider_id` null) inside the migration transaction with a 5 s lock timeout. Undo drilled on a real schema built from migrations 0000-0042 with a full fingerprint (tables, columns, policies, indexes, constraints): restored exactly.

**Code-only rollback.** Safe. The previous build never names the columns; riders would see every delivery again (the old behaviour) and assignments made meanwhile are simply ignored. The undo script drops the columns and loses only the assignments (export first if needed).

**Behaviour change to expect at the door.** After this release a RIDER login sees and can close ONLY deliveries assigned to them; an unassigned delivery is on no rider's list until a manager assigns one (orders board or Deliveries page). Before, every rider saw every ready delivery.

**4j addendum (review fixes, rider assignment).** Security and red-team both found (1) HIGH: `/app/orders` required only a signed-in staff member, so a RIDER who typed the address received every open order with customer phones and addresses (pre-existing; now gated by `mayOpenOrdersBoard`, checked before any order is read; every other staff page was checked and has a real permission gate); (2) `failDelivery` could leave a FAILED order with captured money when a payment landed while it waited on the order lock, and ignored partially refunded/refunded payments: the guard now lives inside `advanceOrder`'s locked transaction for FAILED (same set as the cancellation guard). Both fail-first. Logged, not shipped: `rider-rls-scope` (migration 0042 still lets a RIDER read every order through PostgREST/Realtime, including `rider_id`; a `rider_id = auth.uid()` restrictive policy for rider-only logins is a privilege/RLS change for the owner to approve), `reassign-race-1` (a manager reassigning while the previous rider is mid-close: cash is recorded against the rider who closed it, order completes), `rider-stale-assign` (a deactivated rider keeps `rider_id`; the delivery is on nobody's list until reassigned), and by design a CASHIER (orders.update) may fail any unpaid delivery with a reason.

## 4k. 0052 `rider_rls_scope` (rider-rls-scope + rider-offer, `release/rc-27-rider-offer`)

**PRIVILEGE / RLS CHANGE (flagged per the owner's rule).** Adds `auth_is_rider_scoped(uuid)` (SECURITY DEFINER, like `auth_has_role`) and six RESTRICTIVE select policies (`<table>_rider_scope`) on orders, order_items, order_item_modifiers, order_events, kitchen_line_status and kitchen_order_pack: a rider-only login (holds RIDER and no role that reads orders for its work: orders.view, orders.create or kitchen.view) reads only rows of deliveries assigned to it (`orders.rider_id = auth.uid()`), at the database level (PostgREST, Realtime). Everyone else is unchanged (`NOT rider_scoped OR ...`). The app reads through Drizzle as `postgres` (bypasses RLS): no page or action depends on it. Generated from `src/domain/rls-read-limits.ts` (`scripts/gen-rls-read-policies.ts 0052`), drift-tested. Owner approved 2026-09-21.

**Also in this release (code only): `delivery.take`** (RIDER; new permission) and rider-offer: an unassigned READY/OUT_FOR_DELIVERY delivery shows to every active rider as "Available" (pickup-level facts only: order number, item count, distance, amount, paid or not; never name, phone, address or coordinates), and "Take it" is one conditional UPDATE (first tap wins). Offers are read through Drizzle (unaffected by the RLS policies).

**Classification.** Additive: one function and six restrictive policies; no table, column, grant, row or existing policy changes. Undo drilled on a real schema built from migrations 0000-0051 (function and policies removed, full schema fingerprint restored exactly, re-apply identical). Each policy proven necessary (dropping any one fails the login-simulation test).
...
## 4n. 0055 `client_grant_hardening` (security lows release, `release/rc-34-security-lows`)

**PRIVILEGE CHANGE (flagged per the owner's rule). No data is touched, no table is rewritten, no RLS policy changes.** Three cards in one migration (`pin-anon-grant`, `pin-hash-column`, `orders-column-limits`): `anon` loses SELECT on `memberships` (it kept a table-level SELECT including `pos_pin_hash`; harmless while no anon policy exists) and on `orders`; `authenticated` keeps SELECT on `orders` for every column EXCEPT the customer contact fields `customer_name`, `customer_phone`, `delivery_address`, `delivery_lat_micro`, `delivery_lng_micro`, `notes` (a row policy cannot limit columns, so a KITCHEN, RIDER or ANALYST login could read them through PostgREST). Same mechanism as `memberships` in 0042; the lists live in `src/domain/rls-read-limits.ts` (`ORDERS_READABLE_COLUMNS`, `ORDERS_CONTACT_COLUMNS`) and a test forces a decision for every new `orders` column. `pos_pin_hash` stays a column on `memberships` (moving it to its own table was judged too invasive for a hot table with no PIN feature yet); the grant test now proves neither `anon` nor any signed-in login can read it.

**Why this is safe for the app.** The app reads and writes through Drizzle as `postgres`, which grants do not affect. Nothing reads `orders` or `memberships` through PostgREST (the browser subscribes to `order_events`, `products`, `product_availability`); verified by search and by the RLS suite. **Stricter than the card asked:** the owner and manager lose direct PostgREST access to the contact fields too, because a column grant is per database role, not per app role; they read them in the app as before.

**Classification.** Additive in effect (only removes client read paths). Production grants were read (read-only, 22 Sep 2026) and equal the local ones: `anon` REFERENCES, SELECT, TRIGGER on both tables; `authenticated` table-level SELECT on `orders`, column-level (9 of 10, no `pos_pin_hash`) on `memberships`. **Undo drilled** on a schema built from 0000-0054 with a fingerprint that includes column and table grants: undo exact, re-apply identical. Undo: `supabase/rollback/0055_client_grant_hardening.down.sql` (restores the table-level SELECTs). **Code-only rollback:** safe, no code depends on the grants.

## 4o. 0056 `rider_positions` (Lane B: rider live position, `release/rc-35-rider-live`)

**Additive, one new empty table, safe before the deploy:** `rider_positions` holds one row per GPS fix a rider's browser posts (about every 15 seconds) while a delivery it holds is OUT_FOR_DELIVERY. Same posture as `notification_outbox` (0050) but stricter, matching its own sensitivity: RLS forced, `ALL` privileges revoked from `PUBLIC`, `anon` and `authenticated`, and no client policy at all — nothing can read or write it through PostgREST or Realtime; the app reads and writes it as `postgres` through Drizzle, after `src/lib/repositories/rider-tracking.ts` checks who is asking (a rider posts only for a delivery they hold that is out; a customer reads only the newest fix of an order they placed, only while it is out). Two integer-microdegree columns with the same CHECKs every other coordinate in this schema uses, plus a non-negative accuracy CHECK. Cascades from `orders` and `organizations`.

**Retention.** Rows are deleted after 24 hours by the new `rider-positions-purge` job (hourly at :17, `deploy/frybird-job-rider-positions-purge.timer`, installed the same way as the refund healer's timer). Nothing else in the schema reads this table, so a rider losing signal for a while, or the purge job being late, changes nothing about the delivery itself — only the customer's map goes from "live" to "not available right now" (`src/lib/delivery/tracking.ts`'s `trackerState`, never a stale dot shown as current, per BUILD-PLAN §62).

**Undo drilled** on a schema built from 0000-0055 with a fingerprint that includes RLS flags, grants and policies: undo exact, re-apply identical, zero client grants confirmed after apply. Undo: `supabase/rollback/0056_rider_positions.down.sql` (drops the table; loses at most 24 hours of position history, nothing else). **Code-only rollback:** safe — the app runs unchanged with the table absent, since nothing reads or writes it until a delivery goes OUT_FOR_DELIVERY and a rider's browser starts posting; a stale build hitting the table after a rollback would 42P01 on a database call already wrapped the way every repository call is.
