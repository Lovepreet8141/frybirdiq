...
| `PrinterBridge.kt` | The nine allowed operations (`CAPABILITIES`, `STATUS`, `DISCOVER`, `CONNECT`, `DISCONNECT`, `TEST_CONNECTION`, `TEST_PRINT`, `PRINT_RECEIPT`, `LAST_ERROR`), address validation (private IPv4 only), payload caps, duplicate-job protection. |
...
- No `addJavascriptInterface`. No exposed shell, filesystem, Bluetooth or
  arbitrary sockets. The only socket ever opened is TCP to an address
  that passed the private-IPv4 check, on a validated port.
