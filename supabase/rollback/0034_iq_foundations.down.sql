...
-- policies and grants (all go with the tables), and the four trigger functions.
--
...
-- The journal-row delete is outside this transaction: if it is forgotten,
-- Drizzle believes 0034 is still applied and never re-runs it.
...
DROP FUNCTION IF EXISTS public.iq_actions_stamp_status();
DROP FUNCTION IF EXISTS public.iq_recommendations_reference_insights();
DROP FUNCTION IF EXISTS public.iq_insights_keep_referenced();
