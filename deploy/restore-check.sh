...
grep -c "ERROR" /tmp/restore-check.log || true
grep "ERROR" /tmp/restore-check.log | grep -v "auth\." | head -5 || true

sudo -u postgres dropdb "$SCRATCH_DB"
echo "==> scratch database dropped; restore check passed $(date -u +%H:%M:%SZ)"
