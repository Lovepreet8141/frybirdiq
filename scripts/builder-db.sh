...
BASE=$(grep -E '^DATABASE_URL=' "$WT/.env.test.local" 2>/dev/null | head -1 | sed -E 's#^DATABASE_URL=##; s#^(postgres(ql)?://[^/]+/).*#\1#' || true)
...
PD -c "GRANT USAGE ON SCHEMA public TO anon, authenticated; GRANT SELECT ON ALL TABLES IN SCHEMA public TO anon, authenticated" >/dev/null
